...
import "./MascotaList.css";
 ...
 async function LlenarLista2(dato,archivo)  {
  

//alert(documen.getElementById("selprov").value);

  const response = await fetch('../json/'+ archivo);
  const names = await response.json();

  console.log("Total de localidades =" + names["localidades"].length);

 


  /*for(var i = 0; i< names["localidades"].length; i++){
    //alert(names["provincias"][i]["nombre_completo"])
    dato.innerHTML += ("<option >" + names["localidades"][i]["nombre"] + "</option>"); 
    }*/
    
  }


async function LlenarLista(dato,archivo)  {
  
   if(dato.innerHTML.length > 0) return;

  const response = await fetch('../json/'+ archivo);
  const names = await response.json();

  for(var i = 0; i< names["provincias"].length; i++){
    //alert(names["provincias"][i]["nombre_completo"])
    dato.innerHTML += ("<option >" + names["provincias"][i]["nombre_completo"] + "</option>"); 
    }
  }
    ...
    <div className="mascota-list container-fluid">
      ...
      <div className="row justify-content-center justify-content-md-between text-center container-filtros px-lg-5">
       ...
       {/*  https://datos.gob.ar/dataset/jgm-servicio-normalizacion-datos-geograficos */}
        ...
        <div className="col-5 barra-filtros align-items-center justify-content-center py-10 text-left">
           <p className="filtros-title">Filtros de ubicación</p>
           Provincia 
           <select className="Select" id="selprov"  
                  
                  onFocus={LlenarLista(document.getElementById("selprov"),"provincias.json")}
           ...
           <p></p>
           Localidad 
           <select className="Select" id="selLoc" onFocus={LlenarLista2(document.getElementById("selLoc"),"localidades.json")}>
